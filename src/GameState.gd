...
var effect_damage := false
var effect_miss := false
var effect_critical := false
...
func get_stage_intro_message() -> String:
	match game_stage:
		1:
			return "市場の外れに、黒い影が現れた……。"
		2:
			return "川辺にひそむ影が、静かに近づいてくる……。"
		3:
			return "町外れの土道に、不吉なざわめきが満ちる……。"
		4:
			return "廃墟の門前に、中ボスが立ちはだかる……！"
		5:
			return "茶器がほぼ完成し、力が高まり始めている……。"
		6:
			return "闇の力が暴走し、攻撃が効かない……。"
		7:
			return "最終決戦。光を取り戻す戦いが始まる……！"
		_:
			return ""
...
func player_attack(player_choice: int) -> Dictionary:
	...
	if round_result == 1 or round_result == 2:
		# ステージ6：必殺技前はプレイヤー勝ちでもダメージ0（3回まで）
		if game_stage == 6 and not boss_hp_drain_active and round_result == 1:
			ineffective = true
			damage = 0

			boss_no_damage_count += 1
			if boss_no_damage_count >= 3:
				# 3回効かない攻撃 → HPドレイン開始フラグON
	...
	# フォーム2補正（プレイヤー勝ち時）
	if meirin_form == 2 and round_result == 1 and damage > 0 and not miss and not ineffective:
		damage += 1
	...
	# ダメージ適用
	# -------------------------------
	if round_result == 1 and not ineffective:
		enemy_hp -= damage
	elif round_result == 2:
		player_hp -= damage
	...
	# -------------------------------
	effect_miss = miss
	effect_critical = (round_result == 2 and critical and damage > 0)
	effect_damage = (round_result == 2 and damage > 0)
